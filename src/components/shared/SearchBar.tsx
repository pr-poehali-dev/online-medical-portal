    ...
    <>
      <div className={`relative flex items-center ${className}`}>
        <div className={`flex items-center w-full bg-white rounded-2xl shadow-xl border border-white/60 overflow-hidden ${large ? "p-2" : "p-1.5"}`}>
          ...
          <button
            onClick={openModal}
            className={`gradient-brand text-white font-semibold rounded-xl flex-shrink-0 flex items-center gap-2 transition-all hover:opacity-90 shadow-md ${large ? "px-6 py-3 text-base" : "px-4 py-2 text-sm"}`}
          >
